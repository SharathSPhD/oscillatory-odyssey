"""Interactive pendulum simulation interface with dynamic visualization."""
...
# Import video saving functionality
from .video_saver import save_animation_as_video

class InteractivePendulum:
    """Interactive interface for pendulum simulation visualization."""
    ...
    def setup_interface(self):
        ...
        # Save button description is set in widgets.py
    ...
    def save_animation(self, _=None):
        ...
        if hasattr(self, 'results'):
            ...
            # Create output path
            output_path = "pendulum_animation.mp4"
            ...
            with self.output:
                ...
                # Save the animation
                try:
                    result = save_animation_as_video(self.anim_fig, self.results, output_path)
                    save_status.value = result.value
                    
                    # Also display the figures again to maintain UI state
                    self.display_figures()
                except Exception as e:
                    save_status.value = f"<div style='color: red; font-weight: bold;'>Error saving video:</div><div>{str(e)}</div>"
                    
                    # Import diagnostic info if there was an error
                    try:
                        import sys
                        import imageio
                        save_status.value += f"<div>Python: {sys.version}</div>"
                        save_status.value += f"<div>imageio: {imageio.__version__}</div>"
                        
                        try:
                            import imageio_ffmpeg
                            ffmpeg_path = imageio_ffmpeg.get_ffmpeg_exe()
                            save_status.value += f"<div>ffmpeg found at: {ffmpeg_path}</div>"
                        except Exception as ffmpeg_err:
                            save_status.value += f"<div>Error checking ffmpeg: {str(ffmpeg_err)}</div>"
                    except Exception as diag_err:
                        save_status.value += f"<div>Error getting diagnostic info: {str(diag_err)}</div>"
...
def create_interactive_pendulum():
    """Create and display an interactive pendulum simulation."""
